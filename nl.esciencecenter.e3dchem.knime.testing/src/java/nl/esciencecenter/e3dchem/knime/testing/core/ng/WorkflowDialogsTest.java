 ...
 */
public class WorkflowDialogsTest extends WorkflowTest {
    private static final NodeLogger LOGGER = NodeLogger.getLogger(WorkflowDialogsTest.class);

    public WorkflowDialogsTest(final String workflowName, final IProgressMonitor monitor, final WorkflowTestContext context) {
    ...
    private void checkDialogs(final ErrorCollector collector, final WorkflowManager wfm) throws InterruptedException,
            ...
            if (node instanceof SingleNodeContainer) {
                if (((SingleNodeContainer)node).isInactive()) {
                    ...
                    if (exRef.get() != null) {
                        ...
                        collector.addError(new Throwable(msg, exRef.get()));
                    }
                }
            } else if (node instanceof WorkflowManager) {
                checkDialogs(collector, (WorkflowManager)node);
            } else {
